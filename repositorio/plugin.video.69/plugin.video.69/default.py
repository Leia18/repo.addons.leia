...
addon = xbmcaddon.Addon('plugin.video.69')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.69-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(69 ha fallado. Código de error - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(69, fallo del servidor. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(69,Nueva fuente añadida.,5000,"+icon+")")
        if not url is None:
...
def getChannelItems(name,url,fanart):
        soup = getSoup(url)
        channel_list = soup.find('channel', attrs={'o101' : name.decode('utf-8')})
...
def getSubChannelItems(name,url,fanart):
        soup = getSoup(url)
        channel_list = soup.find('subchannel', attrs={'o101' : name.decode('utf-8')})
...
def getItems(items,fanart,dontLink=False):
        ...
        for item in items:
            ...
            try:
                url = []
                if len(item('i000oo1')) >0:
                    #print 'item link', item('i000oo1')

                    for i in item('i000oo1'):
            ...
            try:
                ...
                if len(url) > 1:
                    ...
                    if isXMLSource:
                        ...
                        try:
                            if 'OO1001o110Oo111' in name and not getRegexParsed==None:
                        ...
                        try:
                            if 'OO1001o110Oo111' in thumbnail and not getRegexParsed==None:
...
def parse_regex(reg_item):
                try:
                    regexs = {}
                    for i in reg_item:
                        regexs[i('o101')[0].string] = {}
                        regexs[i('o101')[0].string]['o101']=i('o101')[0].string
                        #regexs[i('o101')[0].string]['e10lx'] = i('e10lx')[0].string
                        try:
                            regexs[i('o101')[0].string]['e10lx'] = i('e10lx')[0].string
                            if not regexs[i('o101')[0].string]['e10lx']:
                                regexs[i('o101')[0].string]['e10lx']=''
                        except:
                            addon_log("Regex: -- No Referer --")
                        regexs[i('o101')[0].string]['pa'] = i('pa')[0].string
                        try:
                            regexs[i('o101')[0].string]['referer'] = i('referer')[0].string
                        except:
                            addon_log("Regex: -- No Referer --")
                        try:
                            regexs[i('o101')[0].string]['connection'] = i('connection')[0].string
                        except:
                            addon_log("Regex: -- No connection --")

                        try:
                            regexs[i('o101')[0].string]['notplayable'] = i('notplayable')[0].string
                        except:
                            addon_log("Regex: -- No notplayable --")

                        try:
                            regexs[i('o101')[0].string]['noredirect'] = i('noredirect')[0].string
                        except:
                            addon_log("Regex: -- No noredirect --")
                        try:
                            regexs[i('o101')[0].string]['origin'] = i('origin')[0].string
                        except:
                            addon_log("Regex: -- No origin --")
                        try:
                            regexs[i('o101')[0].string]['accept'] = i('accept')[0].string
                        except:
                            addon_log("Regex: -- No accept --")
                        try:
                            regexs[i('o101')[0].string]['includeheaders'] = i('includeheaders')[0].string
                        except:
                            addon_log("Regex: -- No includeheaders --")

                            
                        try:
                            regexs[i('o101')[0].string]['li0lo'] = i('li0lo')[0].string
#                            print 'li0lo',regexs[i('o101')[0].string]['li0lo'],i('li0lo')[0].string, i
                        except:
                        ...
                        try:
                            regexs[i('o101')[0].string]['proxy'] = i('proxy')[0].string
                        except:
                            addon_log("Regex: -- No proxy --")
                            
                        try:
                            regexs[i('o101')[0].string]['x-req'] = i('x-req')[0].string
                        except:
                            addon_log("Regex: -- No x-req --")

                        try:
                            regexs[i('o101')[0].string]['x-addr'] = i('x-addr')[0].string
                        except:
                            addon_log("Regex: -- No x-addr --")                            
                            
                        try:
                            regexs[i('o101')[0].string]['x-forward'] = i('x-forward')[0].string
                        except:
                            addon_log("Regex: -- No x-forward --")

                        try:
                            regexs[i('o101')[0].string]['agent'] = i('agent')[0].string
                        except:
                            addon_log("Regex: -- No User Agent --")
                        try:
                            regexs[i('o101')[0].string]['post'] = i('post')[0].string
                        except:
                            addon_log("Regex: -- Not a post")
                        try:
                            regexs[i('o101')[0].string]['rawpost'] = i('rawpost')[0].string
                        except:
                            addon_log("Regex: -- Not a rawpost")
                        try:
                            regexs[i('o101')[0].string]['htmlunescape'] = i('htmlunescape')[0].string
                        except:
                            addon_log("Regex: -- Not a htmlunescape")


                        try:
                            regexs[i('o101')[0].string]['readcookieonly'] = i('readcookieonly')[0].string
                        except:
                            addon_log("Regex: -- Not a readCookieOnly")
                        #print i
                        try:
                            regexs[i('o101')[0].string]['cookiejar'] = i('cookiejar')[0].string
                            if not regexs[i('o101')[0].string]['cookiejar']:
                                regexs[i('o101')[0].string]['cookiejar']=''
                        except:
                            addon_log("Regex: -- Not a cookieJar")
                        try:
                            regexs[i('o101')[0].string]['setcookie'] = i('setcookie')[0].string
                        except:
                            addon_log("Regex: -- Not a setcookie")
                        try:
                            regexs[i('o101')[0].string]['appendcookie'] = i('appendcookie')[0].string
                        except:
                            addon_log("Regex: -- Not a appendcookie")

                        try:
                            regexs[i('o101')[0].string]['ignorecache'] = i('ignorecache')[0].string
                        except:
...
def getRegexParsed(regexs, url,cookieJar=None,forCookieJarOnly=False,recursiveCall=False,cachedPages={}, rawPost=False, cookie_jar_file=None):#0,1,2 = URL, regexOnly, CookieJarOnly
        ...
        #cachedPages = {}
        #print 'url',url
        doRegexs = re.compile('\OO1001o110Oo111\[([^\]]*)\]').findall(url)
        ...
        for k in doRegexs:
            if k in regexs:
                ...
                if  'cookiejar' in m: # so either create or reuse existing jar
                    ...
                    if  'OO1001o110Oo111' in cookieJarParam:
...
#                        print 'complete_path',complete_path
                        saveCookieJar(cookieJar,cookie_jar_file)
                if  m['pa'] and 'OO1001o110Oo111' in m['pa']:
                    pg=getRegexParsed(regexs, m['pa'],cookieJar,recursiveCall=True,cachedPages=cachedPages)
                    if len(pg)==0:
                        pg='http://regexfailed'
                    m['pa']=pg

                if 'setcookie' in m and m['setcookie'] and 'OO1001o110Oo111' in m['setcookie']:
                    m['setcookie']=getRegexParsed(regexs, m['setcookie'],cookieJar,recursiveCall=True,cachedPages=cachedPages)
                if 'appendcookie' in m and m['appendcookie'] and 'OO1001o110Oo111' in m['appendcookie']:
                    m['appendcookie']=getRegexParsed(regexs, m['appendcookie'],cookieJar,recursiveCall=True,cachedPages=cachedPages)


                if  'post' in m and 'OO1001o110Oo111' in m['post']:
                    m['post']=getRegexParsed(regexs, m['post'],cookieJar,recursiveCall=True,cachedPages=cachedPages)
#                    print 'post is now',m['post']

                if  'rawpost' in m and 'OO1001o110Oo111' in m['rawpost']:
                ...
                link=''
                if m['pa'] and m['pa'] in cachedPages and not 'ignorecache' in m and forCookieJarOnly==False :
                    #print 'using cache page',m['pa']
                    link = cachedPages[m['pa']]
                else:
                    if m['pa'] and  not m['pa']=='' and  m['pa'].startswith('http'):
                        if '$epoctime$' in m['pa']:
                            m['pa']=m['pa'].replace('$epoctime$',getEpocTime())
                        if '$epoctime2$' in m['pa']:
                            m['pa']=m['pa'].replace('$epoctime2$',getEpocTime2())

                        #print 'Ingoring Cache',m['pa']
                        page_split=m['pa'].split('|')
...
#                            print '..........................connection//////.',m['connection']
                        ...
                        if 'post' in m:
                            ...
                            #    (captcha_challenge,catpcha_word,idfield)=processRecaptcha(m['pa'],cookieJar)
    ...
    #                        print link
                            ...
                            pass
                        cachedPages[m['pa']] = link
                        #print link
                        #print 'store link for',m['pa'],forCookieJarOnly

                        if forCookieJarOnly:
                            return cookieJar# do nothing
                    elif m['pa'] and  not m['pa'].startswith('http'):
                        if m['pa'].startswith('$pyFunction:'):
                            val=doEval(m['pa'].split('$pyFunction:')[1],'',cookieJar,m )
                            if forCookieJarOnly:
                                return cookieJar# do nothing
                            link=val
                            link=javascriptUnEscape(link)
                            ...
                            link=m['pa']
                if '$pyFunction:playmedia(' in m['e10lx'] or 'ActivateWindow'  in m['e10lx'] or 'RunPlugin'  in m['e10lx']  or '$PLAYERPROXY$=' in url  or  any(x in url for x in g_ignoreSetResolved):
                    setresolved=False
                if  'OO1001o110Oo111' in m['e10lx']:
                    m['e10lx']=getRegexParsed(regexs, m['e10lx'],cookieJar,recursiveCall=True,cachedPages=cachedPages)
                  
                if not m['e10lx']=='':
                    #print 'doing it ',m['e10lx']
                    if '$LiveStreamCaptcha' in m['e10lx']:
                        val=askCaptcha(m,link,cookieJar)
                        #print 'url and val',url,val
                        url = url.replace("OO1001o110Oo111[" + k + "]", val)

                    elif m['e10lx'].startswith('$pyFunction:') or '#$pyFunction' in m['e10lx']:
                        #print 'expeeeeeeeeeeeeeeeeeee',m['e10lx']
                        val=''
                        if m['e10lx'].startswith('$pyFunction:'):
                            val=doEval(m['e10lx'].split('$pyFunction:')[1],link,cookieJar,m)
                        else:
                            val=doEvalFunction(m['e10lx'],link,cookieJar,m)
                        if 'ActivateWindow' in m['e10lx'] or 'RunPlugin' in m['e10lx']  : return '',False
                        if forCookieJarOnly:
                            return cookieJar# do nothing
                        if 'li0lo' in m:
                            listrepeat=m['li0lo']                            
                            #ret=re.findall(m['e10lx'],link)
                            #print 'ret',val
...
#                        print 'url k val',url,k,val
                        ...
                        try:
                            url = url.replace(u"OO1001o110Oo111[" + k + "]", val)
                        except: url = url.replace("OO1001o110Oo111[" + k + "]", val.decode("utf-8"))
                    else:
                        if 'li0lo' in m:
                            listrepeat=m['li0lo']
                            #print 'li0lo',m['e10lx']
                        ...
                        if not link=='':
                            #print 'i000oo1',link
                            reg = re.compile(m['e10lx']).search(link)                            
                            try:
                                val=reg.group(1).strip()
                            except: traceback.print_exc()
                        elif m['pa']=='' or m['pa']==None:
                            val=m['e10lx']
...
#                            print 'rawpost'
                    ...
                    url = url.replace("OO1001o110Oo111[" + k + "]",'')
        if '$epoctime$' in url:
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(69,No se puede reproducir una URL vacia,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(69, no se puede reproducir el proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(69,Urlresolver no soporta este dominio. - ,5000)")
        resolver=url
    return resolver
...
def play_playlist(name, mu_playlist,queueVideo=None):
        ...
        if '$$LSPlayOnlyOne$$' in mu_playlist[0]:
            ...
            for i in mu_playlist:
                ...
                elif "OO1001o110Oo111" in mu_playlist[index] :
...
def download_file(name, url):
        ...
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('69','Elige una ruta para guardar archivos.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('69', '¿Quieres añadir este archivo como fuente?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from 69 Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to 69 Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if (not play_list) and not any(x in url for x in g_ignoreSetResolved) and not '$PLAYERPROXY$=' in url:#  (not url.startswith('plugin://plugin.video.f4mTester')):
            if regexs:
                ...
                if '$pyFunction:playmedia(' not in urllib.unquote_plus(regexs) and 'notplayable' not in urllib.unquote_plus(regexs) and 'li0lo' not in  urllib.unquote_plus(regexs) :
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from 69 Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to 69 Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
def d2x(d, root="root",nested=0):
    ...
    for key,vl in d.iteritems():
        vtype = type(vl)
        if nested==0: key='re' #enforcing all top level tags to be named as regex
        if vtype is list: 
            for v in vl:
                v=escape(v)
                xml = ml(v,xml)         
...
#        print 'Not setting setResolvedUrl'
    ...
    data=None
    if regexs and 'li0lo' in urllib.unquote_plus(regexs):
...
#        print 'regexs',regexs
 ...
 #       print 'final regexs',regexs,regexname
    ...
    #                print 'i is ',i, len(obj), len(newcopy)
        ...
        #                                print 'aa'
        #                                print '[' + regexname+'.11ooOO111ooOO1'+str(i+1) + ']'
        #                                print repr(obj[i])
                                        if type(the_value) is dict:
                                            for the_keyl, the_valuel in the_value.iteritems():
                                                if the_valuel is not None:
                                                    ...
                                                    if '[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]' in the_valuel:
                                                        the_valuel=the_valuel.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]', unescape(val))
                                                    the_value[the_keyl]=the_valuel.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + ']', val)
                                            ...
                                            if isinstance(obj,tuple):
                                                ...
                                                try:
                                                    ...
                                                    val= obj
                                            if '[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]' in the_value:
                                                #print 'found DE',the_value.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]', unescape(val))
                                                the_value=the_value.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]', unescape(val))

                                            the_valueO[the_key]=the_value.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + ']', val)
                    ...
                    if isinstance(obj,tuple):
                        ...
                        try:
                            ...
                            val=obj
                    if '[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]' in listrepeatT:
                        listrepeatT=listrepeatT.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + '][DE]',val)
                    listrepeatT=listrepeatT.replace('[' + regexname+'.11ooOO111ooOO1'+str(i+1) + ']',escape(val))
#                    print listrepeatT
                listrepeatT=listrepeatT.replace('[' + regexname+'.11ooOO111ooOO1'+str(0) + ']',str(rnumber)) 
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(69,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(69,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(69,Bloqueo Parental Desactivado,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(69, ¿Error Pin?,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(69,Bloqueo parental activado,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
